...
class Capacitacion {
    ...
    public function __construct() {
        $this->conn = Database::getConnection(); // Tu clase de conexión a BD
    }
    ...
    public function crear($data) {
        ...
        return $stmt->execute([$data['nombre'], $data['descripcion'], $data['fecha']]);
    }

    public function actualizar($id, $data) {
        $stmt = $this->conn->prepare("UPDATE capacitaciones SET nombre = ?, descripcion = ?, fecha = ? WHERE id = ?");
        return $stmt->execute([$data['nombre'], $data['descripcion'], $data['fecha'], $id]);
    }
